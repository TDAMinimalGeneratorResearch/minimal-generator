...
% %--------------------- Formatting content
% \input{manuscript_sections/00packages}
% \input{manuscript_sections/01newcommands}
% \input{manuscript_sections/02authors}

% %---------------------

% \begin{document}
% \input{manuscript_sections/03authorformat}
% \input{manuscript_sections/04abstract}
% \input{manuscript_sections/05introduction}
% \input{manuscript_sections/06background} 

% \input{manuscript_sections/07-1RelatedWork}
% \input{manuscript_sections/07-2Methods}
% \input{manuscript_sections/07-3Experiments}
% %\input{manuscript_sections/08notations}
% \input{manuscript_sections/09results}
% \input{manuscript_sections/10discussion}
% \input{manuscript_sections/11miscellaneous}
% \input{manuscript_sections/12figures}
...
\begin{enumerate}
    ...
    %The weight of the edges are taken as the inverse of the correlation between the expression levels of the corresponding genes of the two vertices. 
...
\section{Results and Discussion}\label{results}
 ...
 %In \se \ref{notations}, we first describe the notations we use for discussing the results.
...
% \ref{itm:edge_IL},
% \ref{itm:tri_NIU}, and 
% \ref{itm:tri_IU} in \tab \ref{tab:realworldata} for data described in \se \ref{sec: realworlddata} and \LL{\ref{itm:edge_NIU}, \ref{itm:edge_IU}, \ref{itm:edge_NIL},
% \ref{itm:edge_IL},
% \ref{itm:tri_NIU}, 
...
% We can characterize each of these functions as a form of $\ello$-loss, in the sense that the value taken by $L_\bullet$ on $\optimalrep$ depends only on $\supp(\optimalrep)$.  There is a natural $\ell_1$ counter part to each $L_\bullet$, which we denote $L_\bullet^1$ (for example, $L\EU^1(\optimalrep) = ||\optimalrep||_1$).  We define $C_\bullet ^* = L_\bullet(\optimalrep)$, where $\optimalrep$ is the optimal solution to 

% which weights terms according to the absolute values of the entries of $\optimalrep$, a
...
\fig\ref{fig:lengthcompare} reports ratios on the losses $L\EU$, $L\EL$, and $L_Sur\text{-}Area}$\footnote{Recall, we only compute $L_{Area}$ on the 2-dimensional distribution data.} for the eight $\setofpersistenthcyclebases$ optimization problems detailed in \se \ref{sec:programsandmethods} as well as the four edge-loss $\setoffilteredcyclebases$ problems from \pr \eqref{eq:escolarargmin}, evaluated on the data from Sections \ref{sec: realworlddata} and \ref{sec: randompointclouds}. 
%Then, we divide each loss by the optimum cost attained by each optimization problem in \se \ref{sec:programsandmethods} (when not requiring integer constraints), $C\EU\NI$, $C\EL\NI$, $C\TA\NI$ to study the extent to which the solutions produced by each technique vary.  
...
% The average ratio $\frac{C\NI\EU}{L\EU(\originalrep)}$ is $89.13\%$ for cycle representatives obtained from data described in \se \ref{sec: randompointclouds} and $83.17\%$ for cycle representatives obtained from data described in \se \ref{sec: realworlddata}. The average ratio $\frac{C\NI\EL}{L\EL(\originalrep)}$ is $89.83\%$ for cycle representatives obtained from data described in \se \ref{sec: randompointclouds} and $87.02\%$ for cycle representatives obtained from data described in \se \ref{sec: realworlddata} for cycles obtained from Program \eqref{eq:edgelossgeneral}.
...
%, $\optimalrep_\bullet^*$, and the vertical axis is the ratio between the loss of each type of optimal representative over the actual cost of the optimal representative of the same loss function, where the denominator is fixed across each row to be the $\setofpersistenthcyclebases$ cycles which are solutions to Programs \ref{itm:edge_NIU}, \ref{itm:edge_NIL} (in the case of the first two rows, respectively), or \ref{itm:tri_NIA} (in the last row).
...
%  \textbf{Ambient dimension} & 3 & 3 & 202 &  673 & 688 &  259 & 300 & 261 & 60 & 3 &  1,173\\   
%  \textbf{\# points}   & 400 &  300  &  297 &   1088 &  1397  & 512 & 379 & 445  & 103 & 1,000 & 2,722\\ 
%  \textbf{\# representatives}& 257  & 124  &  107 &174   &  117 (115)   & 438 & 7  & 126  & 12 & 311 & 28 (26) \\  
%  \textbf{$T$ persistence}   & 100.97  & 6.81 & 5.14 &728.51& 967.61     & 143.07 & 12.18  &  9.62 & 0.10 & 1,053.53 & 71,081.77  \\ 
%  [0.5ex] 
% \hline
% \multicolumn{5}{c}{\textbf{\qquad Edge-loss persistent homological cycle representatives (\pr \eqref{eq:edgelossgeneral})}} &&&&&& \\
% \hline
% %  \textbf{Memory (GiB)}    & 5.56 & 3.05 & 199.52 & 181.27  & 7.71 & 72.17 & 7.16 & 0.15& 416.38\\ \hline        
%  \textbf{$T\I\EL$ optimization} & 16.01  & 8.2 &  19.64&466.85  &     & 150.46 & 0.17 & 63.93  & 0.31 & 45.14 & 4,732.59	\\ 
%  \textbf{$T\NI\EL$ optimization} & 11.28  & 6.61 &16.07  &403.63 &     & 86.95 & 0.13 & 48.65  & 0.22 & 34.73 & 4,540.55 \\ 
%  \textbf{$T\I\EU$ optimization} & 14.59  & 9.09 & 19.22 & 473.82 &     & 119.94 & 0.23 &  63.34 & 0.33 & 45.51 & 4,714.90	\\ 
%   \textbf{$T\NI\EU$ optimization} & 11.38  & 5.55 & 15.63 & 404.95 &     & 83.40 & 0.12 & 48.88  & 0.22 & 33.88 & 4,547.37 \\
%   [0.5ex] 
% \hline
% \multicolumn{5}{c}{\textbf{Edge-loss filtered homological cycle represnetatives (\pr \eqref{eq:escolarargmin})}} &&&&&& \\
% \hline
% %  \textbf{Memory (GiB)}    & 5.56 & 3.05 & 199.52 & 181.27  & 7.71 & 72.17 & 7.16 & 0.15& 416.38\\ \hline        
%  \textbf{$T\I\EL$ optimization} & 16.93  &8.64  &20.41  & 468.22 &     &  155.08&  0.17 &  62.20  & 0.30 & 67.77 & 2,999.24	\\ 
%  \textbf{$T\NI\EL$ optimization} & 10.29  & 5.51 &16.15  & 403.74 &     & 88.66 &0.13  &  48.24 & 0.22 & 50.25 & 2,829.12	\\ 
%  \textbf{$T\I\EU$ optimization} & 15.14  & 8.32 &19.76  & 476.84  &     &  142.4&  0.24  & 61.82  & 0.31 & 68.63 & 2,937.16	\\ 
%   \textbf{$T\NI\EU$ optimization} & 11.07  & 5.63 & 16.23 & 406.97 &     & 87.59 &  0.12 & 48.11  &0.22  & 54.05 & 2,833.06	 \\
%   [0.5ex] 
% \hline
% \multicolumn{5}{c}{\textbf{Triangle-loss persistent homological cycle representatives (\pr \eqref{eq:trianglelossgeneral})\qquad}} &&&&&& \\
% \hline
%  \textbf{$T\I\TU$ optimization} & 423.32 & 23.07  &783.06  & 63,227.14 &  &  10,187.41   & 2.71 & 221.07  & 0.26  & 384.91 & 39,140.67 \\ 
%  \textbf{$T\NI\TU$ optimization} & 287.49 & 18.81 & 610.20 &60,459.49 &  &   8,441.76  & 2.08 & 170.95  & 0.23  & 277.93 & 36,401.50  \\ 
%  \textbf{$T$ build all}  & 5.52  & 0.88  & 10.07 & 826.25 &  & 45.28 & 0.05 &  4.85 & 0.03  & 172.60 & - \\ 
% \textbf{Total $T$ build part}  & 6.13 & 3.06  &24.21 & 860.91 &  & 193.99 & 0.26 & 31.37  & 0.03  & 198.27 & 922.50 \\ \hline 
% \end{tabular}
% }%\LL{Updated 03/14}
% }
% \label{tab:realworldata}
